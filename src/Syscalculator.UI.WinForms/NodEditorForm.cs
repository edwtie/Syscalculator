...
public sealed class NodEditorForm : Form
{
    ...
    private sealed class EditorTabHeaderPanel : Panel
    {
        public bool Selected { get; set; }
        public bool Dirty { get; set; }

        public EditorTabHeaderPanel()
        {
            DoubleBuffered = true;
    ...
    private sealed class EditorTabStripPanel : FlowLayoutPanel
    {
        public EditorTabStripPanel()
        {
            DoubleBuffered = true;
            SetStyle(ControlStyles.AllPaintingInWmPaint | ControlStyles.OptimizedDoubleBuffer | ControlStyles.ResizeRedraw | ControlStyles.UserPaint, true);
        }
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            {
                if (control is not EditorTabHeaderPanel { Selected: true })
                    continue;
    ...
    private sealed class EditorTabCloseButton : Button
    {
        private bool _hovered;
        private bool _pressed;

        public EditorTabCloseButton()
        {
            SetStyle(ControlStyles.AllPaintingInWmPaint | ControlStyles.OptimizedDoubleBuffer | ControlStyles.ResizeRedraw | ControlStyles.UserPaint, true);
        ...
        {
            e.Graphics.SmoothingMode = SmoothingMode.AntiAlias;
            var tabBackColor = Parent is EditorTabHeaderPanel { Selected: true }
                ? Color.White
    ...
    public NodEditorForm(string? path = null, bool openTemplateWizard = false)
    {
        ...
        Text = T("editor.title", "NOD Editor Plus");
        Width = 1100;
        Height = 780;
    ...
    private void BuildToolbar()
    {
        ...
        _toolStrip.Items.Add(MakeButton(T("editor.toolbar.new", "New"), (_, _) => AddBlankNewTab(), ToolbarIcon.New));
        _toolStrip.Items.Add(MakeButton(T("editor.toolbar.open", "Open"), Open_Click, ToolbarIcon.Open));
        _saveButton = MakeButton(T("editor.toolbar.save", "Save"), Save_Click, ToolbarIcon.Save);
        _toolStrip.Items.Add(_saveButton);
        _toolStrip.Items.Add(new ToolStripSeparator());
        _toolStrip.Items.Add(MakeButton(T("editor.toolbar.template_wizard", "Templates"), TemplateWizard_Click, ToolbarIcon.Wizard));
        _toolStrip.Items.Add(new ToolStripSeparator());
        _toolStrip.Items.Add(MakeButton(T("editor.toolbar.find", "Find"), (_, _) => ShowFindReplace(), ToolbarIcon.Find));
        _toolStrip.Items.Add(MakeButton(T("editor.toolbar.validate", "Validate"), Validate_Click, ToolbarIcon.Validate));
        _toolStrip.Items.Add(MakeButton(T("editor.toolbar.test", "Test"), Test_Click, ToolbarIcon.Test));
        _toolStrip.Items.Add(MakeButton(T("editor.toolbar.solver_steps", "Solver"), SolverSteps_Click, ToolbarIcon.Solver));
        _toolStrip.Items.Add(MakeButton(T("editor.toolbar.repair", "Repair"), RepairLines_Click, ToolbarIcon.Repair));
    ...
    private static ToolStripButton MakeButton(string text, EventHandler handler, ToolbarIcon icon)
    {
    ...
    private void BuildLayout()
    {
        ...
        _editorTabStrip = new EditorTabStripPanel
        {
            Dock = DockStyle.Fill,
            WrapContents = false,
            AutoScroll = true,
    ...
    private Control BuildGraphPreviewPanel()
    {
        ...
        _graphXMin = MakeGraphNumberBox(-3, -100000, 100000, 1);
        ...
        _graphXMax = MakeGraphNumberBox(3, -100000, 100000, 1);
        ...
        var graphZoomPanel = BuildGraphZoomOverlay();
        ...
        _graphPointTable = BuildGraphPointTable();
        _graphPointPanel = BuildGraphPointOverlayPanel(_graphPointTable);
    ...
    // Maakt een nette puntentabel voor de ingebouwde Graph Preview-tab.
    private static DataGridView BuildGraphPointTable()
    {
        ...
        table.ColumnHeadersDefaultCellStyle.Font = new Font("Segoe UI", 7.5f, FontStyle.Bold);
        table.ColumnHeadersDefaultCellStyle.Padding = new Padding(1, 0, 1, 0);
        table.DefaultCellStyle.Padding = new Padding(1, 0, 1, 0);
        ...
        table.Columns[1].Width = 62;
        return table;
    }

    private Panel BuildGraphPointOverlayPanel(DataGridView table)
    {
        const int tableWidth = 132;
        const int tableHeight = 64;
        const int chromePadding = 5;
        const int titleHeight = 18;
        var overlay = new Panel
        {
            Width = tableWidth + chromePadding * 2,
            Height = titleHeight + 64 + chromePadding * 2,
            BackColor = Color.White,
            Padding = new Padding(chromePadding)
        };
        overlay.Paint += RoundedGraphOverlayPanel_Paint;
        overlay.MouseDown += GraphPointPanel_MouseDown;
        overlay.MouseMove += GraphPointPanel_MouseMove;
        overlay.MouseUp += GraphPointPanel_MouseUp;

        _graphPointTitleBar = new Panel
        {
            Location = new Point(chromePadding, chromePadding),
            Size = new Size(tableWidth, titleHeight),
            BackColor = Color.FromArgb(239, 246, 255),
            Cursor = Cursors.SizeAll
        };
        _graphPointTitleBar.Paint += (_, e) => DrawPointTitleBar(e.Graphics, _graphPointTitleBar.ClientRectangle, T("editor.graph.points", "Points"), _graphPointerText);
        _graphPointTitleBar.MouseDown += (_, e) =>
        {
            if (GetPointTitleCloseRect(_graphPointTitleBar.ClientRectangle).Contains(e.Location))
            {
                SetGraphPointTableVisible(false);
                return;
            }

            GraphPointPanel_MouseDown(_graphPointTitleBar, e);
        };
        _graphPointTitleBar.MouseMove += GraphPointPanel_MouseMove;
        _graphPointTitleBar.MouseUp += GraphPointPanel_MouseUp;

        table.Dock = DockStyle.None;
        table.Location = new Point(chromePadding, chromePadding + titleHeight);
        table.Size = new Size(tableWidth, tableHeight);
        table.MouseDown += GraphPointPanel_MouseDown;
        table.MouseMove += GraphPointPanel_MouseMove;
        table.MouseUp += GraphPointPanel_MouseUp;

        overlay.Controls.Add(_graphPointTitleBar);
        overlay.Controls.Add(table);

        return overlay;
    }

    private Control BuildGraphZoomOverlay()
    {
        var panel = new Panel
        {
            Width = 32,
        ...
        panel.Paint += RoundedGraphOverlayPanel_Paint;
        panel.ControlAdded += (_, e) =>
        {
            if (e.Control is null)
                return;

            var index = panel.Controls.Count - 1;
            e.Control.Location = new Point(3, 3 + index * 31);
        };
        panel.Controls.Add(MakeGraphOverlayButton("⟳", T("editor.graph.reset_view", "Reset view"), (_, _) => ResetGraphPreviewView()));
        panel.Controls.Add(MakeGraphOverlayButton("+", T("editor.graph.zoom_in", "Zoom in"), (_, _) => ZoomGraphPreview(0.75f)));
        panel.Controls.Add(MakeGraphOverlayButton("-", T("editor.graph.zoom_out", "Zoom out"), (_, _) => ZoomGraphPreview(1.35f)));
        return panel;
    }
    ...
    private static void DrawPointTitleBar(Graphics graphics, Rectangle rect, string title, string detail = "")
    {
        ...
        graphics.DrawString(title, font, titleBrush, new Rectangle(rect.Left + 4, rect.Top, rect.Width - 22, rect.Height), format);
        if (!string.IsNullOrWhiteSpace(detail))
            graphics.DrawString(detail, detailFont, detailBrush, new Rectangle(rect.Left + 45, rect.Top, rect.Width - 65, rect.Height), format);

        var closeRect = GetPointTitleCloseRect(rect);
        using var closePen = new Pen(Color.FromArgb(15, 63, 143), 1.7f)
        {
            StartCap = LineCap.Round,
            EndCap = LineCap.Round
        };
    ...
    {
        return new Rectangle(rect.Right - 18, rect.Top, 18, rect.Height);
    }

    private static Control MakeGraphOverlayButton(string text, string tooltip, EventHandler click)
    {
        var button = new GraphRoundOverlayButton
        {
            Text = text,
            Width = 26,
            Height = 26,
    ...
    private sealed class GraphRoundOverlayButton : Control
    {
        private bool _hover;
        private bool _down;

        public GraphRoundOverlayButton()
        {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            if (Text != "+" && Text != "-")
            {
                var arcRect = new RectangleF(centerX - 6.5f, centerY - 6.5f, 13f, 13f);
                e.Graphics.DrawArc(symbolPen, arcRect, 35, 285);
                using var arrowBrush = new SolidBrush(ForeColor);
                e.Graphics.FillPolygon(arrowBrush,
                [
                    new PointF(centerX - 1.7f, centerY - 8.1f),
                    new PointF(centerX + 4.5f, centerY - 7f),
                    new PointF(centerX + 1f, centerY - 2.1f)
                ]);
                return;
            }

            const float half = 5f;
            e.Graphics.DrawLine(symbolPen, centerX - half, centerY, centerX + half, centerY);
            if (Text == "+")
                e.Graphics.DrawLine(symbolPen, centerX, centerY - half, centerX, centerY + half);
        }
    }
    ...
    private void UpdateGraphPointerStatusVisibility()
    {
        ...
        _graphPointerStatusPanel.Visible = !_graphPointPanel.Visible && !string.IsNullOrWhiteSpace(_graphPointerText);
    }
    ...
    private static void RoundedGraphOverlayPanel_Paint(object? sender, PaintEventArgs e)
    {
    ...
    {
        if (_graphPreviewPoints.Count == 0)
        {
            _graphHasView = false;
            if (invalidate)
                _graphCanvas.Invalidate();
        ...
        var minX = (float)_graphXMin.Value;
        var maxX = (float)_graphXMax.Value;
        var minY = _graphPreviewPoints.Min(p => p.Y);
        var maxY = _graphPreviewPoints.Max(p => p.Y);
    ...
    {
        return GraphPlotRenderer.GetPlotRectangle(_graphCanvas);
    }

    private PointF ScreenToGraphPoint(PointF screenPoint, Rectangle plot)
    {
        return GraphPlotRenderer.ScreenToGraph(screenPoint, plot, new GraphPlotView(_graphViewMinX, _graphViewMaxX, _graphViewMinY, _graphViewMaxY));
    }
    ...
    private static void DrawGraphGrid(Graphics g, Rectangle plot)
    {
        using var borderPen = new Pen(Color.FromArgb(203, 216, 234), 1);
        g.DrawRectangle(borderPen, plot);
    }

    private void DrawGraphAxes(Graphics g, Rectangle plot, float minX, float maxX, float minY, float maxY, float requestedMinX, float requestedMaxX, float requestedStep, Func<PointF, PointF> map)
    {
        ...
        var xTicks = IsSameGraphRange(minX, maxX, requestedMinX, requestedMaxX)
            ? BuildAxisTicks(requestedMinX, requestedMaxX, requestedStep, maxTicks: 9)
            : BuildNiceTicks(minX, maxX, maxTicks: 8);
        var yTicks = BuildNiceTicks(minY, maxY, maxTicks: 6);
        ...
        var xAxisY = minY <= 0 && maxY >= 0 ? map(new PointF(minX, 0)).Y : plot.Bottom;
        var yAxisX = minX <= 0 && maxX >= 0 ? map(new PointF(0, minY)).X : plot.Left;
        ...
        foreach (var xValue in xTicks)
        {
            ...
            if (Math.Abs(xValue) < 0.000001f && minX <= 0 && maxX >= 0 && minY <= 0 && maxY >= 0)
                continue;

            var xPoint = map(new PointF(xValue, minY));
            g.DrawLine(tickPen, xPoint.X, xAxisY - 4, xPoint.X, xAxisY + 4);
            var text = FormatAxisTick(xValue);
            var size = g.MeasureString(text, tickFont);
            var labelX = Math.Clamp(xPoint.X - size.Width / 2, plot.Left + 2, plot.Right - size.Width - 2);
            var labelY = Math.Clamp(xAxisY + 5, plot.Top + 2, plot.Bottom - size.Height - 2);
            g.DrawString(text, tickFont, labelBrush, labelX, labelY);
        }

        foreach (var yValue in yTicks)
        {
            if (Math.Abs(yValue) < 0.000001f && minX <= 0 && maxX >= 0 && minY <= 0 && maxY >= 0)
                continue;

            var yPoint = map(new PointF(minX, yValue));
            g.DrawLine(tickPen, yAxisX - 4, yPoint.Y, yAxisX + 4, yPoint.Y);
            var text = FormatAxisTick(yValue);
            var size = g.MeasureString(text, tickFont);
            var labelX = Math.Clamp(yAxisX - size.Width - 6, plot.Left + 2, plot.Right - size.Width - 2);
        ...
        if (minX <= 0 && maxX >= 0 && minY <= 0 && maxY >= 0)
        {
            ...
            var text = "0";
            var size = g.MeasureString(text, tickFont);
            var labelX = Math.Clamp(origin.X - size.Width - 7, plot.Left + 2, plot.Right - size.Width - 2);
            var labelY = Math.Clamp(origin.Y + 5, plot.Top + 2, plot.Bottom - size.Height - 2);
            g.DrawString(text, tickFont, labelBrush, labelX, labelY);
        }

        g.DrawString("x", Font, labelBrush, plot.Right - 12, Math.Clamp(xAxisY - 14, plot.Top + 2, plot.Bottom - 16));
        g.DrawString("y", Font, labelBrush, Math.Clamp(yAxisX - 18, plot.Left + 2, plot.Right - 14), plot.Top + 2);
    }

    private static string FormatAxisTick(float value)
    {
    ...
    private static bool IsSameGraphRange(float minA, float maxA, float minB, float maxB)
    {
        return Math.Abs(minA - minB) < 0.0001f && Math.Abs(maxA - maxB) < 0.0001f;
    }

    private static void NormalizeGraphRange(ref float minX, ref float maxX, ref float minY, ref float maxY, bool padX = true)
    {
        if (padX)
        {
    ...
    private static IReadOnlyList<float> BuildAxisTicks(float min, float max, float step, int maxTicks)
    {
    ...
    private (Panel Panel, Label Title, Button CloseButton) CreateEditorTabHeader(TabPage page)
    {
        var panel = new EditorTabHeaderPanel
        {
            AutoSize = false,
            Width = 170,
            Height = 25,
        ...
        var closeButton = new EditorTabCloseButton
        {
            Text = string.Empty,
            Size = new Size(16, 16),
        ...
        title.Click += (_, _) => SelectEditorTab(page);
        panel.Click += (_, _) => SelectEditorTab(page);
        panel.Controls.Add(title);
        panel.Controls.Add(closeButton);
        return (panel, title, closeButton);
    }
    ...
    private void RefreshEditorTabStrip()
    {
        foreach (var tab in _tabs.Values)
        {
            ...
            if (tab.HeaderPanel is EditorTabHeaderPanel headerPanel)
            {
                headerPanel.Selected = selected;
                headerPanel.Dirty = tab.Dirty;
                headerPanel.Invalidate();
            }
            tab.HeaderTitle.Text = tab.Page.Text;
            tab.HeaderTitle.ForeColor = selected ? Color.Black : Color.FromArgb(40, 55, 75);
            tab.HeaderTitle.Font = new Font(Font, selected ? FontStyle.Bold : FontStyle.Regular);
        }
    ...
    private void AddNewTab(string title, string text, string? path, bool dirty)
    {
        ...
        editor.TextChanged += (_, _) =>
        {
            ...
            SetTabDirty(tab, !string.Equals(editor.Text, tab.CleanText, StringComparison.Ordinal));
    ...
    private void RepairLines_Click(object? sender, EventArgs e)
    {
        ...
        tab.Editor.Text = NodTextNormalizer.NormalizeForEditor(tab.Editor.Text, repairConcatenated: true);
        SetTabDirty(tab, !string.Equals(tab.Editor.Text, tab.CleanText, StringComparison.Ordinal));
        ...
        => HelpLanguageCode() switch
        {
            "eng" => BuildLocalizedHistoryHelp(
                ...
                new[]
                {
                    ...
                    ("Syscal.NET experiment", "Later, the old VB6 code was converted automatically to .NET. That was not the right route: forms, API calls, control arrays, <code>App.Path</code>, <code>Load</code>/<code>Unload</code> and default properties caused too many errors. The developer got extra repair work instead of a clean result."),
            ...
            "deu" => BuildLocalizedHistoryHelp(
                ...
                new[]
                {
                    ...
                    ("Syscal.NET-Experiment", "Eine automatische Umwandlung von VB6 nach .NET war keine gute Route. Forms, API-Aufrufe, Control-Arrays, <code>Load</code>/<code>Unload</code> und Default Properties verursachten zu viele Fehler."),
            ...
            "fra" => BuildLocalizedHistoryHelp(
                ...
                new[]
                {
                    ...
                    ("Experience Syscal.NET", "La conversion automatique de VB6 vers .NET n'etait pas la bonne route. Forms, appels API, control arrays, <code>Load</code>/<code>Unload</code> et proprietes par defaut creaient trop d'erreurs."),
            ...
            "ita" => BuildLocalizedHistoryHelp(
                ...
                new[]
                {
                    ...
                    ("Esperimento Syscal.NET", "La conversione automatica da VB6 a .NET non fu la strada giusta. Forms, chiamate API, control arrays, <code>Load</code>/<code>Unload</code> e proprieta predefinite crearono troppi errori."),
            ...
            "spa" => BuildLocalizedHistoryHelp(
                ...
                new[]
                {
                    ...
                    ("Experimento Syscal.NET", "La conversion automatica de VB6 a .NET no fue buena ruta. Forms, llamadas API, control arrays, <code>Load</code>/<code>Unload</code> y propiedades por defecto causaron demasiados errores."),
            ...
            "por" => BuildLocalizedHistoryHelp(
                ...
                new[]
                {
                    ...
                    ("Experiencia Syscal.NET", "A conversao automatica de VB6 para .NET nao foi boa rota. Forms, chamadas API, control arrays, <code>Load</code>/<code>Unload</code> e propriedades por defeito causaram erros demais."),
            ...
            "ind" => BuildLocalizedHistoryHelp(
                ...
                new[]
                {
                    ...
                    ("Eksperimen Syscal.NET", "Konversi otomatis dari VB6 ke .NET bukan jalan yang baik. Forms, API calls, control arrays, <code>Load</code>/<code>Unload</code>, dan default properties memberi terlalu banyak error."),
            ...
            "zho" => BuildLocalizedHistoryHelp(
                ...
                new[]
                {
                    ...
                    ("Syscal.NET 实验", "后来尝试把旧 VB6 代码自动转换到 .NET，但 forms、API 调用、control arrays、<code>Load</code>/<code>Unload</code> 和默认属性带来太多错误。"),
    ...
    private void UpdateUiState()
    {
        ...
        var title = T("editor.title", "NOD Editor Plus");
