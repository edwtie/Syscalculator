...
public MainForm(string? startupNodPath = null)
{
  ...
  private static bool TryParseNumber(string text, out decimal value)
{
    ...
    if (string.IsNullOrWhiteSpace(_inputTextBox.Text))
    {
        ...
        try
        {
            ...
            var engineInput = NormalizeForDocument(document, _inputTextBox.Text);
            _inputTextBox.Tag = engineInput;
            ...
            _updatingText = true;
            _outputTextBox.Tag = outputText;
            _outputTextBox.Text = FormatNumericForDisplay(outputText);
            _updatingText = false;
    ...
    if (string.IsNullOrWhiteSpace(_outputTextBox.Text))
    {
        ...
        {
            var document = _currentDocument;
            var engineInput = NormalizeForDocument(document, _outputTextBox.Text);
            ...
            _updatingText = true;
            _inputTextBox.Tag = result.Text;
            _inputTextBox.Text = FormatNumericForDisplay(result.Text);
            _updatingText = false;
    ...
    private void WizardExpress_Click(object? sender, EventArgs e)
    {
        ...
        using var form = new WizardExpressForm(_currentDocument);
        ShowOwnedDialog(form);
    }
